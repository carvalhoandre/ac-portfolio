...
- Indexable routes: `/pt-BR/`, `/en/`, three localized case studies per language, and a custom `404.html`.
...
- Compact sticky header; mobile menu blocks page scroll, traps focus, closes with Escape, and closes after navigation.
...
- `npm run test:e2e`: 12 passed across 320 px, 390 px, and desktop
- `npm run build`: 9 public routes plus `404.html` prerendered
- `npm run audit:bundle`: 74.45 kB gzip JS/CSS budget result
...
- The in-app browser integration was unavailable due an environment configuration error; Chrome, Playwright, production screenshots, and Lighthouse were used for equivalent validation.
